...
private:
  ...
  bool check_ordinary(int player, std::pair<int, int> from,
                      std::pair<int, int> to) const;
  bool check_queen(int player, std::pair<int, int> from,
                   std::pair<int, int> to) const;
  bool check_move(int player, std::pair<int, int> from,
                  std::pair<int, int> to) const;
...
public:
  ...
  get_list_of_correct_moves(int player, std::pair<int, int> from) const;
  state check_win() const;

  void save_to_file(std::ofstream &os) const;

  //	friend void XMLCALL dataElement (void *userData, const XML_Char *s, int
  //len);
};
