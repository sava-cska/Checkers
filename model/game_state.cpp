...
number_of_player Game_state::who_moves() const {
  if (type_last == 0) {
    if (who_last == FIRST) return SECOND;
    else return FIRST;
  }
...
void Game_state::move(number_of_player player, board_cell from,
          board_cell to) {
  if (who_moves() != player)
    return;
...
std::vector<board_cell> Game_state::get_list_of_correct_moves(number_of_player player,
...
{
  if (fir.who_last != sec.who_last) return true;
  if (fir.move_to_draw != sec.move_to_draw) return true;
  if (fir.type_last != sec.type_last) return true;
  if (fir.last_move != sec.last_move) return true;
  
  for (int i = 0; i < fir.SIZE; i++)
    for (int j = 0; j < fir.SIZE; j++)
      if (fir.board[i][j] != sec.board[i][j]) return true;
  return false;
}
