...
bool Game_state::check_ordinary(int player, std::pair<int, int> from,
                                std::pair<int, int> to) const {
  if ((player == 2 && board[from.x][from.y] == 'w') ||
      (player == 1 && board[from.x][from.y] == 'b'))
    return false;
  int dx;
  char oth1, oth2;
  if (player == 1)
  ...
  if (to == std::mp(from.x + dx, from.y - 1) ||
      to == std::mp(from.x + dx, from.y + 1))
    return true;
  for (int dx = -1; dx <= 1; dx += 2)
    for (int dy = -1; dy <= 1; dy += 2)
      if (to == std::mp(from.x + dx * 2, from.y + dy * 2) &&
...
bool Game_state::check_queen(int player, std::pair<int, int> from,
                             std::pair<int, int> to) const {
  if ((player == 2 && board[from.x][from.y] == 'W') ||
      (player == 1 && board[from.x][from.y] == 'B'))
    return false;
...
bool Game_state::check_move(int player, std::pair<int, int> from,
                            std::pair<int, int> to) const {
...
bool Game_state::kill(int who, std::pair<int, int> pos) const {
  for (int i = 0; i < SIZE; i++)
    for (int j = 0; j < SIZE; j++) {
      if (!check_move(who, pos, std::mp(i, j)))
        continue;
...
char Game_state::get(int y, int x) { return board[x][y]; }

std::pair<int, int> Game_state::find_kill(int who) const {
  for (int i = 0; i < SIZE; i++)
    for (int j = 0; j < SIZE; j++)
      if (kill(who, std::mp(i, j)))
        return std::mp(i, j);
  return std::mp(-1, -1);
}
...
void Game_state::move(int player, std::pair<int, int> from,
                      std::pair<int, int> to) {
  if (who_moves() != player)
    return;
  ...
  if (player == 1 && to.x == 0 && board[to.x][to.y] == 'w')
    board[to.x][to.y] = 'W';
  if (player == 2 && to.x == 7 && board[to.x][to.y] == 'b')
    board[to.x][to.y] = 'B';
  return;
}

std::vector<std::pair<int, int>>
Game_state::get_list_of_correct_moves(int player,
  ...
  for (int i = 0; i < SIZE; i++)
    for (int j = 0; j < SIZE; j++)
      if (check_move(player, from, std::mp(i, j)))
        pos.pb(std::mp(i, j));
  return pos;
}
    ...
    for (int j1 = 0; j1 < SIZE; j1++)
      for (int i2 = 0; i2 < SIZE; i2++)
        for (int j2 = 0; j2 < SIZE; j2++)
          if (check_move(player, std::mp(i1, j1), std::mp(i2, j2)))
            return GAME;
  if (player == 1)
    return SECOND_WIN;
  else
...
(ptr->state).board[ptr->row][col++] = s[i];
        ...
        {
                for (int i = 0; i < len; i++)
                        if (s[i] == '1' || s[i] == '2') ptr->state.who_last =
s[i] - '0';
        }
        if (ptr->tag == "he_kill")
        {
                for (int i = 0; i < len; i++)
                        if (s[i] == '1' || s[i] == '0') ptr->state.type_last =
s[i] - '0';
        }
        if (ptr->tag == "finish_cell_last_move")
        {
                ...
                for (int i = 0; i < len;)
                {
                        if (s[i] == '-' || (s[i] >= '0' && s[i] <= '9'))
                        {
                                std::string str = "";
                                str += s[i];
                                while (i + 1 < len && s[i + 1] >= '0' && s[i +
1] <= '9')
                                {
                                        i++;
                                        str += s[i];
                                }
                                if (cnt == 0) (ptr->state).last_move.x =
atoi(str.c_str()); else (ptr->state).last_move.y = atoi(str.c_str()); cnt++;
                        }
                        i++;
                }
        }
...
<< " at line "
                                                        <<
XML_GetCurrentLineNumber(parser) << '\n'; XML_ParserFree(parser); assert(0);
                }
        } while (!done);
